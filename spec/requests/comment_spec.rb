...
context "json API" do
  ...
  describe "Create" do
    ...
    let(:user) { FactoryGirl.create :restaurant_owner } #create
    before(:each) do
  ...
  describe "Index" do 
    ...
    let(:user) { FactoryGirl.create :restaurant_owner } #index
    before(:each) do
  ...
  describe "Show" do 
    ...
    let(:user) { FactoryGirl.create :restaurant_owner } #show
    before(:each) do
      headers = { "CONTENT_TYPE" => "application/json" }
      post "/login", '{ "user": { "email": "'+user.email+'", "password": "12345678" } }', headers
      @post = FactoryGirl.create :post
  ...
  describe "Destroy" do 
    ...
    let(:user) { FactoryGirl.create :super_admin } #destroy
    before(:each) do
      headers = { "CONTENT_TYPE" => "application/json" }
      post "/login", '{ "user": { "email": "'+user.email+'", "password": "12345678" } }', headers
      @post = FactoryGirl.create :post
  ...
  describe "Update" do 
    ...
    let(:user) { FactoryGirl.create :restaurant_owner } #update
    before(:each) do
      headers = { "CONTENT_TYPE" => "application/json" }
      post "/login", '{ "user": { "email": "'+user.email+'", "password": "12345678" } }', headers
      @post = FactoryGirl.create :post
    ...
    context "with valid attributes" do
      it "should update the comment" do
        ...
        expect(@post.comments.last.title).to eq("asdfghj")
      end
    ...
    context 'with invalid attributes' do
      it 'does not update the comment' do
        ...
        put "/api/v1/posts/#{@post.id}/comments/#{@post.comments.last.id}?auth_token=#{user.authtokens.first.token}", comment: comment, format: :json
        @post.comments.last.reload 
        expect(@post.comments.last.title).not_to be_empty  
      end
